...
const PREVIEW_PROJECTS_COUNT = 3
...
const parse = filename => {
  ...
  if (lines[0].match(/# text/)) {
    color = markdown.match(/# text: ?(.*)/)[1]
    lines.shift()
  }
...
const templateize = ({ filename, tiles, ...rest }) => {
  ...
  return projectTemplate({ filename, name, tiles: filteredTiles.slice(0, PREVIEW_PROJECTS_COUNT).join("\n"), ...rest });
};
...
const main = async () => {
  ...
  for (let i = 0; i < filenames.length; i++)  {
    ...
    if (filename.match(/md$/)) {
      ...
      console.group();
      const { html, form, color } = parse(filename)
      console.info('--------------------')
      console.info(color)

      const image = filename.split('.')[0] + '.jpg'
