...
const NOTES_OUTPUT_FILE = path.join('code', 'html', 'blog.html')
const NOTE_OUTPUT_PATH = path.join('code', 'html', 'blog')
...
const parseNote = (text) => {
  ...
  marked.lexer(text).forEach((token) => {
    if (token.type === 'heading' && token.depth === 1) {
      ...
      noteObject.authorImage =
        noteObject.author.split(' ')[0].toLowerCase() + '.jpg'
    } else {
      noteObject.content.push(token)
      if (noteObject.summary.length !== 2 && token.type === 'paragraph') {
        noteObject.summary.push(token)
      }
    }
  })

  console.info(noteObject.summary)
